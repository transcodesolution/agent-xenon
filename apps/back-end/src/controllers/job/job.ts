        ...
        const [job] = await Job.aggregate<IPagination<IJob>>([
            ...
            },
            {
                $lookup: {
                    ...
                    foreignField: "jobId",
                    as: "rounds",
                    pipeline: [{ $match: { deletedAt: null } }, { $sort: { roundNumber: 1 } }, { $project: { type: 1, endDate: 1, startDate: 1, qualificationCriteria: 1, roundNumber: 1, mcqCriteria: 1, status: 1, name: 1 } }]
                }
            },
            {
                $lookup: {

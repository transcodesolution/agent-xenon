...
export default function Page() {
  ...
  const [currentQuestion, setCurrentQuestion] =
    useState<IInterviewQuestion | null>(null);
  const [answers, setAnswers] = useState<Record<string, string | string[]>>({});
  const [isSubmitted, setIsSubmitted] = useState(false);

  const {
    data: getInterviewRoundResponse,
    isLoading,
    refetch,
  } = useGetInterviewQuestions({ roundId: id });
  ...
  const roundName =
    getInterviewRoundResponse?.data?.roundName || 'Technical Interview';
  ...
  return (
    <Container size="xl">
      {examStatus && <ExamStatusAlert status={examStatus} />}
      {isSubmitted && (
